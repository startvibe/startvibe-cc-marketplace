#!/bin/bash
# Claude Code 通知插件 - Stop 事件处理器
# 版本: 1.0.0
# 描述: 处理 Claude Code Stop Hook 事件，发送响应完成通知
# 作者: StartVibe 团队
# 许可证: MIT
...
handle_error() {
  local exit_code=$?
  local line_number=$1
  echo "{\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"level\":\"error\",\"script\":\"$SCRIPT_NAME\",\"line\":$line_number,\"exitCode\":$exit_code,\"message\":\"Stop 事件处理失败\"}" >&2
  exit $exit_code
}
...
load_config() {
  ...
  # 返回空配置
  echo '{"version":"1.0.0","enabled":true,"events":{"stop":{"enabled":true,"sound":true,"urgency":"normal"}}}'
}
...
validate_config() {
  ...
  # 检查 Stop 事件配置
  local stop_enabled
  stop_enabled=$(get_config_value "$config" "events.stop.enabled")
  if [[ "$stop_enabled" != "true" ]]; then
    log_debug "Stop 事件已禁用"
    return 1
  fi
...
handle_stop_event() {
  ...
  if [[ -f "$notifier_script" ]]; then
    if "$notifier_script" "$title" "$message" "$urgency" "$sound"; then
      log_info "Stop 事件通知发送成功"
      return 0
    else
      log_error "Stop 事件通知发送失败"
      return 1
    fi
  else
...
main() {
  ...
  # 处理 Stop 事件
  if handle_stop_event "$config"; then
    log_info "Stop 事件处理完成"
    exit 0
  else
    log_error "Stop 事件处理失败"
    exit 1
  fi
}
